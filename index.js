...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@bookshelf.rjcnlsa.mongodb.net/?retryWrites=true&w=majority`;
  ...
  try {
    ...
    app.put("/edit/:id", async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Error occurred while updating product" });
      }
    });
    ...
    app.get("/reading", async (req, res) => {
      ...
      console.log(req.query);
      const result = await ReadingListCollection.find({
        "newdata.email": email,
      }).toArray();
      res.send(result);
    });
